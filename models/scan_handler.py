...
# Accept plain GTIN fallback (variant barcode) if GS1 parsing fails
...
class GS1ScanHandler(models.Model):
    ...
    def handle_scan(self, raw_barcode: str, picking_id: int):
        ...
        # If a specific variant was not resolved and multiple exist, caller should present a selector.
        if not variant and len(tmpl.product_variant_ids) > 1:
            return {
                'status': 'select_variant',
                'template_id': tmpl.id,
                'variants': [{'id': p.id, 'name': p.display_name} for p in tmpl.product_variant_ids],
                'ai': ai,
            }
    ...
    def _apply_scan_to_picking(self, product, picking, ai: dict):
        ...
        return {'status': 'ok', 'move_id': move.id, 'move_line_id': ml.id}
