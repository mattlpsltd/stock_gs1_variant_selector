...
class GS1Helper(models.AbstractModel):
    ...
    def parse_gs1(self, raw: str) -> dict:
        """Lightweight parser for common AIs used here: 01,02,15,37,10 (FNC1-aware)."""
        if not raw:
            return {}
        ai = {}
        s = raw
        ...
        while i < n:
            ...
            if k in ('01', '02'):
                ...
                # Skip unknown AI to next FNC1 (best-effort)
    ...
    def parse_lot(self, ai: dict):
        return ai.get('10') or False

    @api.model
    def get_contained_qty(self, ai: dict):
        q = ai.get('37')
        try:
            return float(q) if q not in (None, '') else 1.0
        except Exception:
            return 1.0
    ...
    def find_template_by_shared_gtin(self, gtin: str):
        ...
        return self.env['product.template'].search([('x_shared_barcode', '=', gtin)], limit=1)
